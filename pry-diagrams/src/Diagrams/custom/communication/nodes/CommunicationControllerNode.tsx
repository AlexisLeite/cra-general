...
export class CommunicationControllerNode extends CommunicationNode {
  readonly communicationNodeType = 'controller' as const;

  constructor(parent: Element | null, state: TNodeConstructorProps) {
    super(parent, {
      ...state,
      box: state.box ?? new Dimensions([0, 0, 200, 100]),
      fill: state.fill ?? '#2b1618',
      stroke: state.stroke ?? '#fda4af',
      label: state.label ?? 'Controller',
    });

    this.classList.add('communication__node--controller');
  }

  protected Render = observer(() => {
    return (
      <div {...customRendererProps(this)}>
        <div className="communication__node-badge">Controller</div>
        <EditableLabel />
      </div>
    );
  });
